...
func (s *Service) Delete(ctx context.Context, providerName string, id uuid.UUID) error {
	...
	// TODO: invalidate cache entries for all members of the deleted group.
	// Member external subjects are not available here without an extra query;
	// this is a known gap — members' cached identities will expire on TTL.
...
func (s *Service) Replace(ctx context.Context, providerName string, id uuid.UUID, in Group) (*Group, error) {
	...
	// TODO: invalidate cache entries for all affected members.
	// Member external subjects are not available here without an extra query;
	// this is a known gap — members' cached identities will expire on TTL.
